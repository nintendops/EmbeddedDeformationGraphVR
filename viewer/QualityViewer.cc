...
calc_mean_curvature()
{
	...
	for (v_it=mesh_.vertices_begin(); v_it!=v_end; ++v_it)
	{
		mesh_.property(vcurvature_, v_it) = 0.0;
		...
		if (!mesh_.is_boundary(v_it.handle()))
		{
			for (vv_it=mesh_.vv_iter(v_it); vv_it; ++vv_it)
			{
				...
				laplace += mesh_.property(eweight_,e) * (mesh_.point(vv_it) - mesh_.point(v_it));
			}
			laplace *= mesh_.property(vweight_, v_it);

			mesh_.property(vcurvature_, v_it) = laplace.norm() * 0.5f;
		}
	}
}
...
calc_uniform_mean_curvature()
{
	...
	for (v_it=mesh_.vertices_begin(); v_it!=v_end; ++v_it)
	{
		mesh_.property(vunicurvature_, v_it) = 0.0;
		laplace = Mesh::Point(0,0,0);

		if (!mesh_.is_boundary(v_it.handle()))
		{
			double n = 0;
			for (vv_it=mesh_.vv_iter(v_it); vv_it; ++vv_it, n++)
			{
				laplace += (mesh_.point(vv_it) - mesh_.point(v_it));
			}
			laplace /= n;

			mesh_.property(vunicurvature_, v_it) = laplace.norm() * 0.5f;
		}
	}
}
...
calc_gauss_curvature()
{
	...
	for (v_it=mesh_.vertices_begin(); v_it!=v_end; ++v_it)
	{
		if (!mesh_.is_boundary(v_it))
		{
			...
			mesh_.property(vgausscurvature_, v_it) =
				(2*M_PI-angles) * 2.0f * mesh_.property(vweight_,v_it);
		}
	}
...
calc_triangle_quality()
{
	...
	for (f_it=mesh_.faces_begin(); f_it!=f_end; ++f_it)
	{
		...
		v0 = mesh_.point(cfvIt.handle());
		v1 = mesh_.point((++cfvIt).handle());
		v2 = mesh_.point((++cfvIt).handle());

		v0v1 = v1-v0;
		v0v2 = v2-v0;
		v1v2 = v2-v1;

		denom = 4.0*(v0v1%v0v2).sqrnorm();
		if (fabs(denom) < FLT_MIN)  circum_radius_sq = FLT_MAX;
		else circum_radius_sq = v0v1.sqrnorm() * v0v2.sqrnorm() * v1v2.sqrnorm() / denom;
		
		min_length_sq = v0v1.sqrnorm();
		if (v0v2.sqrnorm() < min_length_sq) min_length_sq = v0v2.sqrnorm();
		if (v1v2.sqrnorm() < min_length_sq) min_length_sq = v1v2.sqrnorm(); 
		
		mesh_.property(tshape_, f_it) = sqrt(circum_radius_sq/min_length_sq);
	}
}
