...
public:
    ...
    void init(const std::vector< OpenMesh::Vec3f> & _pts);

    /// release data
    void release();

    /// retrieve closest point of query
    int getClosestPoint(const OpenMesh::Vec3f & _queryVertex);
	std::vector<int> getClosestPoints(const OpenMesh::Vec3f & _queryVertex, const int k);
	bool IfInitialized; 
