...
ClosestPoint::
init(
    const std::vector< OpenMesh::Vec3f > & _pts
)
{
    release();
...
ClosestPoint::
getClosestPoint(
        const OpenMesh::Vec3f & _queryVertex
)
{
...
std::vector<int>    // returns index
ClosestPoint::
getClosestPoints(
	const OpenMesh::Vec3f & _queryVertex, const int k
)
{
	
	// initialize ANN types for wrapping
	ANNpoint queryPt;                           // query point
	ANNidxArray nnIdx = new ANNidx[K_NEAREST_NODE + 1];
	ANNdistArray dists = new ANNdist[K_NEAREST_NODE + 1];
	...
	kDTree_->annkSearch(queryPt, K_NEAREST_NODE + 1, nnIdx, dists, 0);
	annDeallocPt(queryPt);
	std::vector<int> rnIdx;

	for (int i = 0; i < K_NEAREST_NODE + 1; ++i)
		rnIdx.push_back(nnIdx[i]);
